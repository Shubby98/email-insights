"""
provider.py
-----------
Provider config and OpenAI-compatible client for local LLM access.

Supports LM Studio and Ollama via the LLM_PROVIDER env var (default: lmstudio).

Env vars:
    LLM_PROVIDER   lmstudio (default) | ollama
    LLM_MODEL      Override the model name
    LLM_BASE_URL   Override the base URL (with or without /v1 suffix)
"""

import os
from pathlib import Path

from dotenv import load_dotenv
from openai import OpenAI

load_dotenv(Path(__file__).parent.parent / ".env")

_PROVIDER_DEFAULTS = {
    "lmstudio": {
        "base_url": "http://127.0.0.1:10101/v1",
        "api_key": "lm-studio",
        "model": "google/gemma-3-4b",
    },
    "ollama": {
        "base_url": "http://localhost:11434/v1",
        "api_key": "ollama",
        "model": "gemma3:4b",
    },
}


def _init() -> tuple[OpenAI, str, str]:
    provider = os.getenv("LLM_PROVIDER", "lmstudio").lower()
    if provider not in _PROVIDER_DEFAULTS:
        raise ValueError(
            f"Unknown LLM_PROVIDER '{provider}'. Choose: {', '.join(_PROVIDER_DEFAULTS)}"
        )
    defaults = _PROVIDER_DEFAULTS[provider]
    base_url = os.getenv("LLM_BASE_URL", defaults["base_url"])
    if not base_url.endswith("/v1"):
        base_url = base_url.rstrip("/") + "/v1"
    model = os.getenv("LLM_MODEL", defaults["model"])
    client = OpenAI(base_url=base_url, api_key=defaults["api_key"], timeout=60.0)
    return client, model, provider


_client, _model, _provider = _init()


def get_client() -> OpenAI:
    return _client


def get_model() -> str:
    return _model


def get_provider() -> str:
    return _provider


def get_base_url() -> str:
    """Return the base URL without /v1 suffix (used by lifecycle for non-OpenAI endpoints)."""
    base = os.getenv("LLM_BASE_URL", _PROVIDER_DEFAULTS[_provider]["base_url"])
    return base.removesuffix("/v1").rstrip("/")
